<template>
    <div class="item-info" @click="openBook">
        ...
        <br>
        <span class="item-body">You have rated the book <strong>{{item.book.title }}</strong> written by <strong class="clickable-text" @click="openAuthor($event)">{{ item.book.author }}</strong> with {{ item.rating }} stars</span>
    </div>
</template>
