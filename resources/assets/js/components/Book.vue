  ...
  <div class="content-wrapper">
    <div class="book" v-if="selectedBook">
      <div class="modal-header">
        ...
        <button v-if="selectedList" @click="setContent('list')">
          <span class="clickable-text">List</span>
        </button>
        <button @click="close">
          <span class="clickable-text">close</span>
        </button>
        <button v-if="hasHistory" @click="back">
          <span class="clickable-text">Back</span>
        </button>
      </div>

      <div class="nav-arrows">
        <span class="prev" @click="previousBook">
          <i class="material-icons">arrow_back</i>
        </span>
        <span class="next" @click="nextBook">
      ...
      <div class="modal-body">
        <div class="book-info">
          ...
          <span class="author" @click="selectAuthor(selectedBook.author.id)">by
            <strong>
          ...
          <div class="ratings">
            <div class="overall">
              <span :style="{'color': colorScheme.background}">
                <strong>{{ selectedBook.overall_rating }}</strong>
              </span>
            ...
            <div class="user" v-if="selectedBook.in_library">
              <i class="material-icons" :key="n" @click="rateBook(n)" v-for="n in 10">{{ userRating >= n ? 'star' : 'star_border' }}</i>
            </div>
        ...
        <div class="book-actions">
          ...
          <button v-if="selectedBook.in_library" @click="setContent('reading-session-list')">
            <span class="clickable-text">Reading Sessions</span>
          </button>
          <button @click="showComments">
            <span class="clickable-text">Comments</span>
          </button>
          <button @click="setContent('stats')">
            <span class="clickable-text">Stats</span>
          </button>
        </div>
      </div>

      <div class="modal-footer">
        <button @click="removeBookFromUserCollection" v-if="selectedBook.in_library">
          <span class="clickable-text">Remove from collection</span>
        </button>
        <button v-if="canDeleteBook" @click="deleteBook">
